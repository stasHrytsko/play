...
## D-016 — Taxi Slide visual redesign: per-game colours hard-coded in `theme.ts`, D-015's CSS override removed as dead, 2026-09-22

**Decision:** applied a visual redesign for `two-moves-later` — real taxi
silhouettes (front/side orientation, windows, wheels) instead of a flat
tinted square with a glyph, a passenger figure instead of a ring-plus-glyph,
a darker board so tiles read clearly, and the target cell under a waiting
passenger highlighted in that passenger's colour. `LevelScene.ts` and
`theme.ts` changed; nothing in `mechanic/engine/` or the shell did.

`theme.ts` now hard-codes `board`, `cell`, `glass`, `glassDivider`, `wheel`,
`skin`, and all three taxi `colors.*.fill` values directly as numeric
literals, instead of reading `ui.surfaceRaised`/`ui.background`/`ui.rose`/
`ui.blue`/`ui.mustard` off `readUiTheme()`. `readTheme(root)` still calls
`readUiTheme(root)` for the fields that do stay token-driven (`background`,
`border`, `text`, `passengerRing`).

**This makes D-015's CSS override dead, and it's removed.** D-015 shadowed
`--piece-rose`/`--piece-blue`/`--piece-mustard` under `.two-moves-later-surface`
so `theme.ts` would pick up different values through the same `ui.rose`/
`ui.blue`/`ui.mustard` read. This redesign stopped reading those fields at
all, so the shadowed values had nothing left to feed — keeping the CSS block
would have been a comment explaining a mechanism no longer in use. The
`readTheme(params.container)` call in `mechanic/index.ts` (D-015's other
half) stays: it's still what keeps `background`/`border`/`text`/`passengerRing`
scoped to this game's own container rather than `document.documentElement`,
and nothing about this redesign needed that to change.

**Where the patch came from:** applied from a `.patch` file the project
owner supplied (generated by a different coding tool, not written in this
session). Reviewed before applying — diffed cleanly against the current
tree, read in full, and verified rather than trusted: typecheck, lint
(architecture boundaries hold — nothing in the diff touches `engine/` or
imports outside what `render/**` is already allowed), test (106/106),
build, e2e (both `two-moves-later` specs), and a phone-viewport screenshot
of the actual rendered level to confirm the visual claim, not just that it
compiles.
